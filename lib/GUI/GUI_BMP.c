...
UBYTE GUI_ReadBmp(const char *path)
{
	...
	{ // fp = 0x00426aa0
		sd_journal_perror("Can't open the file!");
		return 0;
	}
	DEBUG("open:%s", path);
	...
	DEBUG("bBitCount=%d", bmpInfoHeader.bBitCount);		 // Print a pixel takes several bit!!!
	...
	printf("frist add:0x%x",bmpFileHeader.bOffset);
	printf("BMP height:%d",bmpInfoHeader.bHeight);
	printf("BMP width :%d",bmpInfoHeader.bWidth);
	*/
